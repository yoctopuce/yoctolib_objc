/*********************************************************************
 *
 * $Id: yocto_voltage.m 11112 2013-04-16 14:51:20Z mvuilleu $
 *
...
static NSMutableDictionary* _VoltageCache = nil;
...
+(YVoltage*) FindVoltage:(NSString*) func
{
    ...
    // Search in cache
    {
        if (_VoltageCache == nil){
            _VoltageCache = [[NSMutableDictionary alloc] init];
        }
        if(nil != [_VoltageCache objectForKey:func]){
            retVal = [_VoltageCache objectForKey:func];
       } else {
           YVoltage *newVoltage = [[YVoltage alloc] initWithFunction:func];
           [_VoltageCache setObject:newVoltage forKey:func];
           retVal = newVoltage;
